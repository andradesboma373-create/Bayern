...
// set winners (first team wins)
r1.forEach(m => { m.winnerId = m.team1!.id; m.score1 = 2; m.score2 = 0; });
rounds.push(r1);

// R2
let r2 = generateNextSwissRound(teams, rounds, 3, 3)!;
// set winners (first team wins)
r2.forEach(m => { m.winnerId = m.team1!.id; m.score1 = 2; m.score2 = 0; });
rounds.push(r2);

// R3
let r3 = generateNextSwissRound(teams, rounds, 3, 3)!;

console.log("R3 Matches:");
r3.forEach(m => {
    // get score
