...
app.post("/api/matches/clear/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
    if (!userId) {
      return res.status(400).json({ error: "Missing userId" });
    ...
    fallbackDb.deleteAllForUser('matches', userId);
    console.log(`[API] Successfully cleared all matches for user ${userId}`);
    res.json({ success: true });
  } catch (err: any) {
...
// ==========================================
// ROOM AUTH & ABUSE-PROTECTED MANAGEMENT API
// ==========================================
...
app.post("/api/auth/login", (req, res) => {
  try {
    ...
    const result = authenticateRoom(username, password);
    if (!result.success || !result.room) {
      return res.status(401).json({ error: result.error || "Неверный логин или пароль" });
    }
...
// Get All Rooms (Admin bamep only)
...
// Create a New Room (Admin bamep only)
app.post("/api/admin/rooms/create", (req, res) => {
  try {
    const { username, password, channelName, role } = req.body;
    const result = createRoom(username, password, channelName, role);
    if (!result.success || !result.room) {
      return res.status(400).json({ error: result.error || "Не удалось создать комнату" });
    }
    const { password: _, ...safeRoom } = result.room;
    res.json({ success: true, room: safeRoom });
  } catch (err: any) {
...
// Toggle Lock / Freeze Room on abuse or check (Admin bamep only)
app.post("/api/admin/rooms/toggle-lock", (req, res) => {
  try {
    const { roomId, lock, reason } = req.body;
    const result = toggleLockRoom(roomId, !!lock, reason);
    if (!result.success) {
...
// Reset Room Quota & Unlock (Admin bamep only)
app.post("/api/admin/rooms/reset", (req, res) => {
  try {
    const { roomId } = req.body;
    const result = resetRoomQuota(roomId);
    if (!result.success) {
      return res.status(400).json({ error: result.error });
    }
    res.json({ success: true });
...
// Get Audit Logs for Room (Action history, abuse details)
app.get("/api/admin/rooms/:roomId/audit-logs", (req, res) => {
  try {
    const { roomId } = req.params;
    const logs = getRoomAuditLogs(roomId);
