...
export function simulateMatchSeries(
    ...
    const winningTeam = results.team1Score > results.team2Score ? 1 : (results.team2Score > results.team1Score ? 2 : 1);

    const allPlayers = [
        ...results.team1Stats.map((p: any) => ({ ...p, teamNum: 1 })),
        ...results.team2Stats.map((p: any) => ({ ...p, teamNum: 2 }))
    ];
    let mvp = null as any | null, bestScore = -Infinity;
    allPlayers.forEach(p => {
        const isWinner = p.teamNum === winningTeam;
        const hltv = parseFloat(p.hltvRating || '1.0');
        const kd = parseFloat(p.kd || '1.0');
        const adr = parseFloat(p.adr || '70');
        const impact = parseFloat(p.impact || '1.0');
        const rounds = p.totalRounds || 1;
        const kpr = p.kills / rounds;
        
        // Multi-kill & opening duel impact
        const mkScore = ((p.k2 || 0) * 1.5) + ((p.k3 || 0) * 4) + ((p.k4 || 0) * 8) + ((p.k5 || 0) * 15);
        const mkPerRound = mkScore / rounds;
        const fkDiff = ((p.fk || 0) - (p.fd || 0)) / rounds;
        
        // Comprehensive HLTV MVP score
        let score = (hltv * 35.0) + (impact * 25.0) + (kpr * 25.0) + (kd * 8.0) + (adr * 0.15) + (mkPerRound * 10.0) + (fkDiff * 8.0);
        
        // Winner priority boost (standard in competitive tournaments)
        if (isWinner) {
            score *= 1.25;
        }
