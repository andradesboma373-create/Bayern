...
export default function Teams({ user }: { user: any }) {
  ...
  const [newTeamBalance, setNewTeamBalance] = useState<number | ''>('');
  ...
  const hasUnsavedChanges = Object.keys(editingValRatings).length > 0 || Object.keys(editingRatings).length > 0;
  ...
  const handleExportAllTeamsJSON = () => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(teams, null, 2));
    const dlAnchorElem = document.createElement('a');
    dlAnchorElem.setAttribute("href", dataStr);
    dlAnchorElem.setAttribute("download", `teams_backup_${user?.name || 'export'}.json`);
    dlAnchorElem.click();
  };

  const handleImportTeamsJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!window.confirm(`Импортировать ${incomingTeams.length} команд? Существующие команды будут объединены.`)) {
        return;
      }
      ...
      updatedTeams = updatedTeams.map(t => {
        if (t.id === editingTeamId) {
          return {
            ...
            players: roster,
            balance: newTeamBalance === '' ? (t.balance ?? 0) : Number(newTeamBalance),
            leader: newTeamLeader.trim() || t.leader || '',
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-black/40 p-4 md:p-6 rounded-2xl border border-white/5 shadow-lg">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <input 
            ...
            ref={jsonInputRef} 
            accept=".json" 
            onChange={handleImportTeamsJSON} 
            className="hidden" 
          />
          <button
            type="button"
            onClick={handleExportAllTeamsJSON}
            className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer"
            title="Скачать все команды в JSON"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 hover:text-white px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer"
            title="Загрузить команды из JSON файла"
          >
          ...
          <button
            onClick={() => {
              ...
              setNewTeamName('');
              setNewTeamLogo(null);
              setNewTeamIsAcademy(activeTab === 'academy');
              setNewTeamBalance('');
              setNewTeamLeader('');
              setSelectedPlayers(['', '', '', '', '']);
              setShowAddForm(!showAddForm);
            }}
            className="flex items-center gap-2 bg-[#ff8f00] hover:bg-[#ff8f00]/90 text-black px-4 py-2 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(255,143,0,0.3)] justify-center cursor-pointer"
          >
            {showAddForm ? <Trash2 className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
            {showAddForm ? 'Отмена' : (activeTab === 'academy' ? 'Добавить академию' : 'Добавить')}
          </button>
        </div>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={handleSaveTeam} className="bg-black/40 border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div className="md:col-span-3">
              ...
              <div className="flex gap-4">
                ...
                >
                  ...
                  />
                  {(() => {
                    const previewLogo = newTeamLogo || getAutoMatchedVectorLogo(newTeamName);
                    if (previewLogo) {
                      return <img src={previewLogo} className="w-full h-full object-cover" alt="Logo" />;
                    }
                    return <span className="text-[10px] font-black text-white/30 uppercase">Лого</span>;
                  })()}
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  {(() => {
                    const autoLogo = !newTeamLogo && getAutoMatchedVectorLogo(newTeamName);
                    if (autoLogo) {
                      return (
                        <span className="text-[11px] text-emerald-400 font-bold flex items-center gap-1.5 mt-1.5">
                          ✨ Найден официальный векторный логотип! Применится автоматически при сохранении.
                        </span>
                      );
                    }
                    return null;
            ...
            <div className="flex items-center gap-2 h-[50px] px-2 mb-1 mt-6">
              <input
                ...
                className="w-5 h-5 rounded border-white/10 bg-black/50 text-[#ff8f00] focus:ring-[#ff8f00] cursor-pointer"
              />
          ...
          {/* Extra Team Management: Balance & Leader */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 bg-black/30 p-4 rounded-xl border border-white/5">
            <div>
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                <Crown className="w-3.5 h-3.5 text-amber-400" />
                Лидер / Владелец команды
              </label>
              <input
                type="text"
                value={newTeamLeader}
                onChange={e => setNewTeamLeader(e.target.value)}
                placeholder="Никнейм лидера (например: @captain, s1mple)"
                className="w-full bg-black/50 border border-white/10 rounded-xl px-3.5 py-2.5 text-white focus:outline-none focus:border-[#ff8f00] text-sm"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
              ...
              <input
                ...
                onChange={e => setNewTeamBalance(e.target.value === '' ? '' : Number(e.target.value))}
                placeholder="0"
                className="w-full bg-black/50 border border-white/10 rounded-xl px-3.5 py-2.5 text-white focus:outline-none focus:border-emerald-500 text-sm font-mono"
              />
            </div>
          </div>
                  ...
                  <div key={t.id} className="flex flex-col sm:flex-row items-stretch sm:items-center bg-[#1a1a24]/80 hover:bg-[#222230] border-b border-white/5 last:border-0 transition-colors group p-3 md:px-4 md:py-3 gap-3 md:gap-0">
                      ...
                      <div className="cursor-pointer flex-1" onClick={() => setSelectedTeamProfile(t)} title="Открыть профиль команды">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold text-white/40 mt-1">
                          ...
                          {t.leader && (
                            <>
                              <span className="w-1 h-1 bg-white/20 rounded-full"></span>
                              <span className="text-amber-400/90 flex items-center gap-0.5" title={`Лидер: ${t.leader}`}>
                                <Crown className="w-2.5 h-2.5" />
                                <span className="truncate max-w-[80px] sm:max-w-[110px]">{t.leader}</span>
                              </span>
                            </>
                          )}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2 sm:mt-0 sm:ml-4 sm:w-24 md:w-32">
                      ...
                      <button onClick={() => {
                        ...
                        setNewTeamBalance(t.balance !== undefined && t.balance !== null ? t.balance : '');
                        setNewTeamLeader(t.leader || '');
            ...
            <div className="bg-[#12121a] p-8 rounded-2xl max-w-2xl w-full border border-white/10 relative shadow-[0_0_50px_rgba(37,99,235,0.15)] flex flex-col gap-6">
              <div className="flex justify-between items-start">
                ...
                <button 
                  onClick={() => {
                    if (hasUnsavedChanges) {
                      if (!window.confirm("У вас есть несохраненные изменения в рейтингах! Закрыть без сохранения?")) {
                        return;
                      }
                    }
                    setSelectedTeamRoster(null);
