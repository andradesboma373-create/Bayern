...
export default function GslGroupStage({
  tournament,
  onUpdate,
  onAdvanceToBracket,
  onVetoMatch,
  ...
  const getUpperRoundLabel = (rIdx: number, total: number) => {
    ...
    if (advanceCount === 2) {
      if (remaining === 1) return 'Матч победителей за 1-е место (Winners Match)';
      if (remaining === 2) return 'Полуфиналы (Opening Matches)';
  ...
  const getLowerRoundLabel = (rIdx: number, total: number) => {
    ...
    if (advanceCount === 2) {
      if (total === 2) {
        if (rIdx === 0) return 'Матч на вылет (Elimination Match)';
        if (rIdx === 1) return 'Решающий матч за 2-е место (Decider Match)';
      }
      if (remaining === 1) return 'Решающий матч за 2-е место (Decider)';
      if (remaining === 2) return 'Полуфинал нижней сетки';
      return `Раунд ${rIdx + 1} нижней сетки`;
    }
    if (advanceCount === 3 && total === 1) {
      return 'Матч за 3-е место (Выход в плей-офф)';
    }
  ...
  const activeStandings = activeGroup ? getGslGroupStandings(activeGroup, advanceCount) : null;
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="bg-[#ff8f00]/20 text-[#ff8f00] font-black text-xs px-3 py-1 rounded-lg uppercase tracking-wider">
              {advanceCount === 2
                ? '1-й Этап: Классический GSL (Top-2 выходят в Плей-офф)'
                : advanceCount === 3
                ? '1-й Этап: Группы GSL (IEM / BLAST — Top-3 в Плей-офф)'
                : '1-й Этап: Группы GSL (ESL Pro League — Top-4 в Плей-офф)'}
            </span>
            {isAllReady ? (
          ...
          <p className="text-white/60 text-xs mt-2 max-w-2xl leading-relaxed">
            {advanceCount === 2
              ? 'В каждой группе: победитель матча виннеров занимает 1 место (2-0). Проигравший падает в Решающий матч (Decider). Победитель Decider матча занимает 2 место (2-1). 3 и 4 места выбывают.'
              : advanceCount === 3
              ? 'В каждой группе: победитель финала виннеров занимает 1 место (выход в 1/2 финала), проигравший занимает 2 место (выход в 1/4 финала). В нижней сетке разыгрывается 3 место (выход в 1/4 финала). 4 место выбывает.'
              : 'В каждой группе: победитель финала виннеров занимает 1 место (1/4 финала), проигравший 2 место (R2). В нижней сетке разыгрываются 3 и 4 места (R1 плей-офф).'}
          </p>
        </div>
      ...
      {gslGroups.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {gslGroups.map((group, idx) => {
            const st = getGslGroupStandings(group, advanceCount);
        ...
        <div className="flex flex-col gap-8 bg-black/35 backdrop-blur-md p-6 rounded-2xl border border-white/10 shadow-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-3 flex flex-col justify-center">
              ...
              <p className="text-xs text-white/50 mt-1">
                {advanceCount === 2
                  ? 'Сетка группы: Верхняя сетка (Виннера) и Нижняя сетка с Решающим матчем (Decider)'
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                  <span className="text-[10px] text-emerald-400 font-bold block">
                    {advanceCount === 2
                      ? '1-е место (Плей-офф 2-0 ✅)'
                      : advanceCount === 3
                      ? numGroups <= 2 ? '1-е место (1/2 Финала 🚀)' : '1-е место (1/4 Финала 🚀)'
                      : '1-е место (1/4)'}
                  </span>
          ...
          <div className="flex flex-col gap-4">
            <h4 className="text-sm font-black uppercase tracking-widest text-emerald-400">
              {advanceCount === 2
                ? 'Верхняя сетка (Upper Bracket) — Победитель матча виннеров занимает 1-е место (2-0), проигравший падает в Decider Match'
                : advanceCount === 3
          ...
          <div className="flex flex-col gap-4">
            <h4 className="text-sm font-black uppercase tracking-widest text-amber-400">
              {advanceCount === 2
                ? 'Нижняя сетка (Lower Bracket) — Elimination Match + Decider Match (разыгрывается 2-е место)'
                : advanceCount === 3
