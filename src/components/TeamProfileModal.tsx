...
import { X, Trophy, Award, Shield, Users, Download, Edit3, Globe, BarChart3, Swords, Settings, Star, Medal, ExternalLink, Check, Calendar, DollarSign, PieChart, TrendingUp, CheckCircle, Crown } from 'lucide-react';
...
export default function TeamProfileModal({ team, user, onClose, onUpdateTeam, allPlayers = [], allTeams = [] }: TeamProfileModalProps) {
  ...
  const [editLeader, setEditLeader] = useState(team.leader || '');
  const [editBalance, setEditBalance] = useState<number | string>(team.balance !== undefined && team.balance !== null ? team.balance : '');
  const [editIsAcademy, setEditIsAcademy] = useState(!!team.isAcademy);
      ...
      >
        ...
        <div className="p-4 sm:p-6 border-b border-white/10 bg-[#12131e] flex flex-col md:flex-row items-center md:items-start gap-5">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {/* Quick Metrics Bar */}
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 mt-4 bg-black/30 border border-white/5 rounded-xl p-2.5 text-center">
              <div>
              ...
              <div>
                <span className="text-[10px] text-white/40 uppercase font-bold block">Баланс клуба</span>
              ...
              <div>
                <span className="text-[10px] text-white/40 uppercase font-bold block">Лидер клуба</span>
                <span className="text-xs font-bold text-amber-400 truncate block" title={currentTeam.leader || 'Не указан'}>
                  {currentTeam.leader || 'Не указан'}
                </span>
              </div>

              <div>
                <span className="text-[10px] text-white/40 uppercase font-bold block">Тренер</span>
                <span className="text-xs font-bold text-white/80 truncate block">{currentTeam.coach || 'Не указан'}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {activeTab === 'edit' && user?.isCustom && !isDownloading && (
            <div className="space-y-4 bg-[#161726] border border-white/5 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs text-white/60 font-bold block mb-1 flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. @captain, s1mple"
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:border-blue-500 focus:outline-none"
                  />
                </div>
                ...
                <div className="flex items-center gap-2 pt-6">
                  <input
                    type="checkbox"
                    id="edit_team_academy"
                    checked={editIsAcademy}
                    onChange={e => setEditIsAcademy(e.target.checked)}
                    className="w-5 h-5 rounded border-white/10 bg-black/50 text-blue-600 focus:ring-blue-500 cursor-pointer"
                  />
                  <label htmlFor="edit_team_academy" className="text-sm font-bold text-white/70 select-none cursor-pointer">
                    Академия (АКД состав)
                  </label>
                </div>
              </div>
